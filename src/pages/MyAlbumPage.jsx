  ...
  return (
    <div className="min-h-screen w-full bg-gray-950 text-white py-16 px-6 flex flex-col overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto w-full">
        {Object.keys(sectionMap).map((sectionTitle) => {
          const resourceKey = sectionMap[sectionTitle];
          const totalCount = ResourceMap[resourceKey]?.total || 0;
          const collectedCount = progress[resourceKey]?.collected || 0;

          const totalAlbumSlots = Array.from(
            { length: totalCount },
            (_, i) => i + 1
          );
